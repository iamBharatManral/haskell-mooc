...
leftpad s l = spaces ++ s
 where
  spaces = concat $ map (\_ -> " ") [0 .. remaining - 1]
  remaining = l - length s
...
countdown n = "Ready! " ++ output ++ " Liftoff!"
 where
  output = count n
  count 1 = show 1 ++ "... "
...
smallestDivisor n = divisor n 2
 where
  divisor n c = if n `mod` c == 0 then c else divisor n (c + 1)
...
biggestPrimeAtMost n = biggest n
 where
  biggest n = if isPrime n then n else biggest (n - 1)
